...
// Import helper for manageing ints
...
contract Domains is ERC721URIStorage {
  // OpenZeppelin helper to keep track of tokenIds.
  ...
  // New mapping to store values/records
  mapping(string => string) public records;

   // We make the contract "payable" by adding this to the constructor
  constructor(string memory _tld) payable ERC721("Shan Name Service", "SNS") {
    tld = _tld;
  ...
  function register(string calldata name) public payable {
      ...
      // Everything from mapping should point to 0
      // Unless it has already been updated/assigned
      // Here we are checking for that before doing anything else with requre
      // If require fails then we exit out
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Shan name service", "image": "data:image/svg+xml;base64,',
